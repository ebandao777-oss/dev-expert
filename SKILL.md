...
**显式调用优先**：用户输入若以 `@<英文标识>` 开头（如 `@code-generation 帮我实现JWT`），跳过关键词匹配，直接加载该子技能。标识必须严格等于下方「子技能索引」表「英文标识」列中的 18 个合法标识之一，拼写错误自动回退关键词路由。位置约束：`@<标识>` 必须在输入开头或独占一段，邮箱、@提及等不触发。显式调用只跳过路由匹配，意图三分法、安全闸门、澄清策略、验证闭环、复盘沉淀全部照常执行。
...
2. **Decision Record**：关键技术决策（架构选型/修复方案/规范修订）及撤销条件
...
本技能为独立套件，无直接联动的其他职业技能；集成外部数据/服务时使用主 Agent 现有检索工具。
...
> 默认单一 LLM 串行执行（见「三、角色视角自检」）；启用子 Agent（检索/执行/审查型）时，以下 L0 边界不可绕过、不可被子 Agent 自身逻辑覆盖。

#### 2.1 子Agent 分类与定位

| 类型         | 代表                                | 用途                       | 是否允许改项目代码 |
| ------------ | ----------------------------------- | -------------------------- | ------------------ |
| 检索收集型   | code-explorer 等                    | 跨文件/跨目录检索与收集    | 禁止（仅读）       |
| 执行型       | 主 Agent 显式委派的实现子任务       | 在已明确边界内落码         | 仅在主 Agent 授权且已审查边界下 |
| 审查型       | code-reviewer / debugger 等         | 分析输出问题清单/根因      | 禁止（仅分析）     |

#### 2.2 核心禁令（L0 级，不可绕过）

- **禁改码**：任何子 Agent 一律禁止写或改项目代码文件；写码/改码须由主 Agent 自写或经主 Agent 显式授权并复核边界后执行。
- **方案须复核**：子 Agent 产出的修复/修改方案，须主 Agent 审查后再落地（呼应安全闸门与红线6：子进程发现的错误必须逐个审查确认）。
- **不继承铁律**：子 Agent 不继承本技能铁律（版本约束/环境差异/用户有意优化/项目约定等），其生成式结论须主 Agent 复核，禁止直接采信。

#### 2.3 检索收集型子Agent 输出规范

- 仅返回 `文件:行号 + 原文` 的机械收集结果，**禁止推理、归纳或语义总结**。
- 不得输出"建议修复方案"等生成式结论（审查型子 Agent 除外，但其结论仍须主 Agent 复核）。
- 主 Agent 基于返回原文自行推演，不信任子 Agent 的语义归纳；上下文污染风险由主 Agent 把控。

#### 2.4 委派前置条件（理解委派闸门）

- 主 Agent 必须先输出 **目标 / 边界 / 验收口径**，再委派子任务。
- 未输出即委派 → 触发「对话流异常处理边界 → 理解委派」失败模式，收回任务回到需求分析；只在任务明确后委派执行型子任务（详见 `task-decomposition-and-execution.md` 第四步「上下文隔离」与「委派简报」）。
- 子 Agent 仅接收明确任务，不能替主 Agent 判断用户真实意图或业务边界（呼应"不得委派理解"）。

#### 2.5 边界外禁止

- 子 Agent 不得访问/修改 `.ai-memory` 之外的记忆系统，不得执行数据库写入、生产配置、权限变更、凭证处理或绕过安全闸门。
- 子 Agent 发现的问题必须逐个回传主 Agent 审查确认，禁止自行落地或静默闭环。

#### 2.6 与主流程衔接

- 子 Agent 返回后，主 Agent 必须运行验证命令/采集证据，对照验收口径复核（见「对话流异常处理边界 → 跳过复核」失败模式）。
- 未验证即转述子 Agent 结论 → 违反 Step 4 验证前提，视为未完成。

### 三、角色视角自检

> 本技能包由单一 LLM 串行执行，下表角色为思维框架而非多 Agent 协同。执行时按角色视角自检，不强制多 Agent 调度。**若启用子 Agent（检索型/执行型/审查型），其权限与边界必须服从上方「二、子Agent 边界与委派协议」，角色视角自检不替代该协议。**
...
3. 达到安全最大轮次 → 强制结束**当前 Wave 的收敛循环**，输出该 Wave 未完成清单（长任务须转交下一 Wave / 写 `handoff.md` 续做，已验证 Wave 的产出物保留，不回退清零）
...
| 网络/远程服务类 | 2            | 网络卡顿、请求超时、临时 5xx 可间隔重试；涉及写操作前必须确认幂等 |

各 reference 的「失败回退机制」表中如出现与基线冲突的最大重试数，以本基线为准；reference 可在基线范围内细化，但不得超过上限。

#### 错误提示可读性规则

执行失败/报错/测试不通过/触发安全闸门时，必须同时输出技术说明与普通人可读说明：

| 输出项           | 要求                                                                 |
| ---------------- | -------------------------------------------------------------------- |
| 发生了什么       | 用一句话说明失败点，例如“测试没有通过，因为登录接口返回了 500”       |
| 可能原因         | 列 1-3 个候选原因，标注“已确认”或“待验证”                            |
| 修复方向         | 给出下一步最小动作，例如“先看错误日志第 X 行”或“先补数据库字段”      |
| 风险提醒         | 若涉及生产配置、数据库写入、权限、凭证、不可回滚操作，必须先请求确认 |
| 需要用户提供什么 | 只在确实缺少输入时提出，格式为“需 [角色] 提供 [具体输入]”            |

禁止只输出“报错了/失败了/环境问题/请检查配置”这类不可执行提示。

#### 网络重试规则

网络卡顿、接口超时、依赖下载失败或外部 API 临时 5xx 时，最多 2 次自动重试（每次重试前说明原因与等待策略）。以下不得自动重试：

- 请求会产生写入、副作用、扣费、发消息、发邮件、发布上线等不可安全重复操作
- 返回 4xx、权限失败、参数校验失败、业务规则失败
- 用户已要求暂停、停止或只报告状态
- 重试需要新增凭证、修改生产配置或绕过安全校验

重试后仍失败时，输出当前状态、已尝试次数、最后一次错误摘要和可选下一步，不得静默继续扩大执行范围。
...
| 理解委派     | 主 Agent 未输出目标/边界/验收口径即委派子任务                    | 收回任务，回到需求分析；只在任务明确后委派执行型子任务（见「二、子Agent 边界与委派协议」） |
...
长任务须保证中途可恢复、结束有可靠产出物。本规则约束 Agent 自身的长链路执行（与所设计的 Init→Step→Poll 产品系统为两层，见对应 reference），两层都须有可验证进度与恢复点。

- **轮次上限仅约束单 Wave 收敛循环**：「安全最大轮次 = 6」是防止单个 Wave 内无限迭代的硬上限，**不跨 Wave 累计**。真正的大任务应通过 Wave 拆分 + 跨会话 `handoff.md` 续做；已完成 Wave 的产出物（变更文件 + 验证证据）不得因后续 Wave 触顶轮次上限而被丢弃或"回退清零"。
- **周期性检查点（checkpoint）**：长任务每完成一个 Wave，或每累计完成 5 个原子任务，必须向 `.ai-memory/handoff.md`（跨会话可迁移）或本任务状态文件落检查点，包含：`已完成项(带验证证据)` → `相关文件` + `未完成项(下一步动作)` + `阻塞项`。目的：上下文被压缩/崩溃后可无损续做，不重跑已验证部分。
- **两层进度都须可验证**：
  - 被设计的系统层：`Init` 创建、`Step` 分批、`Poll` 查询进度、`Cancel` 取消、`失败重试` / `僵尸检测` 必须各有验证证据（见 `software-project.md` 第六步「长任务验证」证据表）。
  - Agent 执行层：每个原子任务完成后必须产出自检 `Task Summary`（完成状态/修改文件/验证证据/置信度/偏差/遗留问题），未经主 Agent 复核不得计入完成度。
- **可靠交付物强制项（呼应 Step 5）**：长任务最终交付必须显式包含四块，未验证项**不得隐藏**：
  1. `已完成清单`：每项带验证证据（命令+输出 / 测试报告 / API 响应 / 人工确认）
  2. `未验证项 / 已知限制`：无法验证或降级处理的必须明示，禁止把"已返回结果"等同于"已验证完成"
  3. `部分失败明细`（若长任务存在分批失败）：哪些批次成功、哪些失败、是否可 `resume`、失败原因与已落地的错误列表
  4. `下一步`：未完成项的续做动作与所需输入
- **上下文污染防护**：长任务执行中只加载当前 Wave / 当前原子任务所需上下文（见「二、子Agent 边界与委派协议 → 2.3」与 `task-decomposition-and-execution.md` 第四步），已完成 Wave 的细节仅以 `previous_summary` 形式传递，不回灌完整对话历史。
...
| 长任务可靠 | 长任务是否落检查点、是否披露未验证项/部分失败  | 有 handoff/状态文件检查点；交付含未验证项与部分失败明细，无隐藏 |
...
**未验证项强制披露（L0）**：交付中凡无法验证、降级处理或长任务部分失败的条目，必须在「已知限制/未验证项/部分失败明细」中显式列出，**禁止隐藏、禁止用"已完成"掩盖未实测项**；长任务另须附「已完成(带证据) + 未完成(下一步) + 检查点位置(handoff.md/状态文件)」。未声明证据类型的验证视为未完成（见 Step 4）。
...
常见疑问、执行禁区清单、验证失败处置、边界外请求和显式调用答疑集中维护在 [FAQ.md](./FAQ.md)。遇到不确定情况时优先查阅该文件。
