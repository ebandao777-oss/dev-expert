...
面向软件工程师和开发团队的编程全生命周期助手。覆盖从软件项目总控、网站项目总控、需求分析、前端设计、MySQL数据库、代码生成、Bug诊断到重构、测试、文档、上线交付和运维监控的全链路，通过18个子技能提供专业化支持。特别强化PHP+MySQL CMS二次开发场景，提供CMS自动探测、PHP版本选型、数据库规范、安全红线、AJAX渐进式防卡死等专项指引。
