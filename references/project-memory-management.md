...
### 第二步补充：代码锚定决策（@decision 注释）

Decision Record 长文承载「为什么」，但靠 agent 自觉写、覆盖率低、与代码脱锚（见 `project-knowledge-graph` 的「@decision 注释约定」）。补充机制：在**代码处**写 `@decision` 注释（如 hack / 反模式 / 非标准结构 / 绕路依赖处），由 `build_graph.py` 抽为图谱 `decisions` 锚点，跨会话查图谱即见「代码长啥样 + 为什么这样写」，且零 LLM token、随代码自动沉淀、不依赖 agent 写。

- **何时写**：改代码遇到非显然决策（技术选型 / 兼容取舍 / 绕路 hack / 反模式保留）时，**顺手在代码处加一行 `@decision <ID> <理由>`**（轻量，本就给人看）。
- **写法**：`// @decision DEC-1 为何选A：兼容性约束放弃B（详见 Decision Record: 决策名）`；ID 与 Decision Record 同名或对应。
- **与 Decision Record 互补**：`@decision` 是代码处锚点（轻、随代码、自动抽取）；Decision Record 是长文（重、含选项分析/撤销条件/影响范围）。注释引 Decision Record 名，Decision Record 的「影响文件」反引 `@decision DEC-1`，互引不重叠。
- **不替代**：`@decision` 只记锚点摘要，复杂决策的长文+撤销条件**仍须**落 Decision Record，不可只写注释。
...
### 第三步点七：知识资产分层（可共享 / 内部 / 敏感）

记忆内容按**可见范围**分层，避免内部信息外泄或可复用知识被埋没：

| 层 | 内容 | 存放 | 判定 |
| - | - | - | - |
| 可共享 | 规范、约定、通用坑、架构决策（不含内部标识） | 项目记忆，可随代码库提交 | 对外发布无风险 |
| 内部 | 业务规则细节、系统拓扑、模块负责人 | 项目记忆（**不入公开仓库**） | 含内部结构信息 |
| 敏感 | 凭证、密钥、生产 IP、账号、个人信息 | **禁止写入项目记忆**（只记「存在何处、如何获取」） | 命中即拒绝写入 |

- 写入前对每条内容问一句：「这条进了对外仓库会有风险吗？」命中 → 降层或改写。
- 跨项目复用（换机 / 换团队）时只带**可共享层**；内部层随项目走，敏感层不迁移。
- 与 `error-ledger.md` 禁忌一致：错误册同样禁记凭证类信息。
...
**四字段精简版（Wave 边界 / 每 5 原子任务落盘，追加进任务状态文件的 Checkpoint 区块）**：
...
- 四字段为上方九字段的**精简子集**：Wave 级高频续做用它（省篇幅、随 checkpoint 追加），换机 / 换 IDE / 跨会话完整交接用九字段；两者兼容不互覆，可追加进同一 Checkpoint 区块。
- 与压缩快照互补：压缩快照管「压缩即时性」（当前文件 / 卡点），交接四字段管「Wave 级续做」。
...
记忆膨胀/冲突/透明度不足时按时间、证据强度、风险等级收敛，旧规则标注过期。失败模式与主 SKILL.md「对话流异常边界」对齐。
