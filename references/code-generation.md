...
- **加载项目规范 + 强制风格嗅探**：从项目记忆中加载相关编码规范（命名约定、错误处理模式等）；并在生成/修改前按 `style-alignment.md` 协议**先读取目标项目已有的代表性文件**，提取缩进/开标签/括号位置/数组键引号/模板变量命名等风格标记，产出「风格基线」块，生成代码逐行对齐——**禁止凭 Agent 自身想象套用风格**（二开/插件开发场景强制）
...
- 单项标注"无需处理"须写明理由（如"纯计算函数无 IO，无重试需求"），禁止留白
